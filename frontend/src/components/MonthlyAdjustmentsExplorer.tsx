  ...
  return (
    <Card className="md:col-span-1">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg">Revenue Adjustments Explorer</CardTitle>
        <CardDescription>
      ...
      <CardContent className="pt-2">
        ...
        <div className="bg-gray-50 rounded-lg border p-4 mb-4">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <h3 className="text-xl font-bold text-gray-800">{selectedMonth}</h3>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center border-b pb-2">
              <span className="font-medium">Original Billed:</span>
              <span className="text-blue-600 font-medium">{formatCurrency(selectedMonthData.originalBilled)}</span>
            </div>
            
            <div className="flex justify-between items-center text-red-600">
              <span className="font-medium">Contractual Adjustments:</span>
              <span className="font-medium">{formatCurrency(selectedMonthData.contractualAdjustments)}</span>
            </div>
            
            <div className="flex justify-between items-center text-orange-600">
              <span className="font-medium">Denials:</span>
              <span className="font-medium">{formatCurrency(selectedMonthData.denials)}</span>
            </div>
            
            <div className="flex justify-between items-center text-yellow-600">
              <span className="font-medium">Other Adjustments:</span>
              <span className="font-medium">{formatCurrency(selectedMonthData.otherAdjustments)}</span>
            </div>
            
            <div className="flex justify-between items-center text-purple-600">
              <span className="font-medium">Write-offs:</span>
              <span className="font-medium">{formatCurrency(selectedMonthData.writeOffs)}</span>
            </div>
            
            <div className="flex justify-between items-center border-t border-b py-2 text-red-600 font-semibold">
              <span>Total Adjustments:</span>
              <span>{formatCurrency(selectedMonthData.totalAdjustments)}</span>
            </div>
            
            <div className="flex justify-between items-center text-green-600 font-semibold">
              <span>Net Revenue:</span>
              <span>{formatCurrency(selectedMonthData.netRevenue)}</span>
            </div>
            
            <div className="flex justify-between items-center">
              <span className="font-medium">Adjustment Rate:</span>
              <span className="font-medium">{selectedMonthData.adjustmentRate}%</span>
            </div>
          </div>
          
          {/* Month-over-month comparison */}
          <div className="mt-6 pt-4 border-t">
            <h4 className="text-sm font-semibold text-gray-600 mb-2">Month-over-Month Change</h4>
            <div className="flex justify-between items-center">
              <span className="text-sm">Net Revenue:</span>
              <span className="text-sm font-medium text-green-600">+3.2% from previous month</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm">Adjustment Rate:</span>
              <span className="text-sm font-medium text-red-600">+1.5% from previous month</span>
            </div>
          </div>
        </div>
